...
class ConfigurationSettingsType(type):
    ...
    specification = namedtuple("ConfigurationSettingSpecification", ("type", "constraint", "supporting_protocols"))

    # P1 [ ] TODO: Review ConfigurationSettingsType.specification_matrix for completeness and correctness

    specification_matrix = {
        "clear_required_fields": specification(type=bool, constraint=None, supporting_protocols=[1]),
        ...
        "maxinputs": specification(
            type=int, constraint=lambda value: 0 <= value <= six.MAXSIZE, supporting_protocols=[2]
        ),
        "overrides_timeorder": specification(type=bool, constraint=None, supporting_protocols=[1]),
        "required_fields": specification(type=(list, set, tuple), constraint=None, supporting_protocols=[1, 2]),
        ...
        "streaming_preop": specification(type=(bytes, six.text_type), constraint=None, supporting_protocols=[1, 2]),
        "type": specification(
...
class RecordWriter(object):
    ...
    def _record_count(self):
        warnings.warn(
            "_record_count will be deprecated soon. Use pending_record_count instead.", PendingDeprecationWarning
        )

...
class AsyncBoltSocket:
    ...
    async def _handshake(self, resolved_address, deadline):
        ...
        log.debug(
            "[#%04X]  C: <HANDSHAKE> %s %s %s %s", local_port, *supported_versions
        )
        ...
        if data_size == 0:
            ...
            await self.close()
            raise ServiceUnavailable(
                f"Connection to {resolved_address} closed without handshake " "response"
            )
        if data_size != 4:
    ...
    @classmethod
    async def connect(
        cls, address, *, tcp_timeout, deadline, custom_resolver, ssl_context, keep_alive
    ):
        ...
        if not errors:
            ...
            raise ServiceUnavailable(
                ...
                % (
                    str(address),
                    tuple(map(str, failed_addresses)),
                    "\n".join(map(str, errors)),
                )
            ) from errors[0]
    ...
    def connect(
        ...
        if not errors:
            ...
            raise ServiceUnavailable(
                ...
                % (
                    str(address),
                    tuple(map(str, resolved_addresses)),
                    "\n".join(map(str, errors)),
                )
            ) from errors[0]

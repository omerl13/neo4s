...
class Bolt:
    ...
    def protocol_handlers(cls, protocol_version=None):
        ...
        handlers = {Bolt3.PROTOCOL_VERSION: Bolt3, Bolt4x0.PROTOCOL_VERSION: Bolt4x0}
    ...
    def open(cls, address, *, auth=None, timeout=None, **pool_config):
        ...
        if pool_config.protocol_version == (3, 0):
            ...
            raise BoltHandshakeError(
                "The Neo4J server does not support communication with this driver. This driver have support for Bolt Protocols {}".format(
                    supported_versions
                ),
                address=address,
    ...
    def _acquire(self, address, timeout):
        ...
        with self.lock:
            ...
            while True:
                ...
                infinite_pool_size = (
                    self.pool_config.max_connection_pool_size < 0
                    or self.pool_config.max_connection_pool_size == float("inf")
                )
                can_create_new_connection = (
                    infinite_pool_size
                    or len(connections) < self.pool_config.max_connection_pool_size
                )
...
class Neo4jPool(IOPool):
    ...
    @classmethod
    def open(
        cls, *addresses, auth, pool_config, workspace_config, routing_context=None
    ):
    ...
    def __init__(
        self, opener, pool_config, workspace_config, routing_context, addresses
    ):
        """
        ...
        self.routing_tables = {
            workspace_config.database: RoutingTable(
            ...
            with self._acquire(address, timeout) as cx:
                ...
                if cx.PROTOCOL_VERSION == Bolt3.PROTOCOL_VERSION:
                    if database != DEFAULT_DATABASE:
                        raise ConfigurationError(
                            "Database name parameter for selecting database is not supported in Bolt Protocol {!r}. Database name {!r}. Server Agent {!r}.".format(
    ...
    def update_routing_table_from(self, *routers, database=None):
        ...
        """
        log.debug(
            "Attempting to update routing table from {}".format(
        ...
        for router in routers:
            ...
            if new_routing_table is not None:
                ...
                log.debug(
                    "[#0000]  C: <UPDATE ROUTING TABLE> address={!r} ({!r})".format(
                        router, self.routing_tables[database]
                    )
                )
                return True
        return False
    ...
    def ensure_routing_table_is_fresh(self, *, access_mode, database):
        ...
        if self.routing_tables[database].is_fresh(
        ...
        with self.refresh_lock:
            ...
            for database in list(self.routing_tables.keys()):
                ...
                if (
                    self.routing_tables[database].should_be_purged_from_memory()
...
def _handshake(s, resolved_address):
    ...
    supported_versions = [
        ("0x%02X%02X%02X%02X" % (vx[0], vx[1], vx[2], vx[3])) for vx in handshake
    ]

    log.debug(
        "[#%04X]  C: <MAGIC> 0x%08X",
        local_port,
        int.from_bytes(Bolt.MAGIC_PREAMBLE, byteorder="big"),
    )

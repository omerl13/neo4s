...
def dehydrate_time(value):
    ...
    if value.tzinfo:
        return Structure(b"T", nanoseconds, int(value.tzinfo.utcoffset(value).total_seconds()))
    else:
...
if np is not None:
    ...
    def dehydrate_np_datetime(value):
        ...
        if not 0 < year <= 9999:
            ...
            raise ValueError(f"Year out of range ({MIN_YEAR:d}..{MAX_YEAR:d}) " f"found {year}")
        seconds = value.astype(np.dtype("datetime64[s]")).astype(int)
        nanoseconds = value.astype(np.dtype("datetime64[ns]")).astype(int) % NANO_SECONDS
        return Structure(b"d", seconds, nanoseconds)
...
if np is not None:
    ...
    def dehydrate_np_timedelta(value):
        ...
        return dehydrate_duration(Duration(nanoseconds=value.astype("timedelta64[ns]").astype(int)))

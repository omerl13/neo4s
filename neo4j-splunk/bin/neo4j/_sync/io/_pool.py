...
class IOPool(abc.ABC):
    ...
    def _acquire_from_pool_checked(self, address, health_check, deadline):
    ...
    def _acquire_new_later(self, address, auth, deadline):
        def connection_creator():
            ...
            try:
                try:
                    connection = self.opener(address, auth or self.pool_config.auth, deadline)
    ...
    def _acquire(self, address, auth, deadline, liveness_check_timeout):
        ...
        while True:
            ...
            connection = self._acquire_from_pool_checked(address, health_check, deadline)
            if connection:
                log.debug(
                    "[#%04X]  _: <POOL> picked existing connection %s", connection.local_port, connection.connection_id
                )
                try:
                    self._re_auth_connection(connection, auth, force_auth)
                except ConfigurationError:
                    if auth:
            ...
            with self.lock:
                ...
                if timeout == 0 or not self.cond.wait(timeout):  # deadline expired
    ...
    @abc.abstractmethod
    def acquire(self, access_mode, timeout, database, bookmarks, auth: AcquireAuth, liveness_check_timeout):
    ...
    def on_neo4j_error(self, error, connection):
        ...
        if error._has_security_code():
            handled = Util.callback(connection.auth_manager.handle_security_exception, connection.auth, error)
            if handled:
...
class BoltPool(IOPool):
    ...
    def acquire(self, access_mode, timeout, database, bookmarks, auth: AcquireAuth, liveness_check_timeout):
        ...
        return self._acquire(self.address, auth, deadline, liveness_check_timeout)
...
class Neo4jPool(IOPool):
    ...
    def fetch_routing_info(self, address, database, imp_user, bookmarks, auth, acquisition_timeout):
    ...
    def fetch_routing_table(self, *, address, acquisition_timeout, database, imp_user, bookmarks, auth):
    ...
    def _update_routing_table_from(
        ...
        for router in routers:
            for address in NetworkUtil.resolve_address(router, resolver=self.pool_config.resolver):
                ...
                if new_routing_table is not None:
                    ...
                    old_routing_table = self.get_or_create_routing_table(new_database)
    ...
    def acquire(self, access_mode, timeout, database, bookmarks, auth: t.Optional[AcquireAuth], liveness_check_timeout):

...
_TWrappedIter = t.TypeVar("_TWrappedIter", bound=t.Callable[..., t.AsyncIterator])
...
class AsyncNonConcurrentMethodChecker:
    if ENABLED:
        ...
        def __make_error(self, tbs):
            msg = f"Methods of {self.__class__} are not concurrency " "safe, but were invoked concurrently."
            if tbs:
        ...
        def non_concurrent_method(cls, f: _TWrapped) -> _TWrapped:
            if AsyncUtil.is_async_code:
                ...
                if not callable(f):
                    raise TypeError(
                        "cannot decorate non-callable object with " "NonConcurrentMethodChecked.non_concurrent_method"
                    )
        ...
        def non_concurrent_iter(cls, f: _TWrappedIter) -> _TWrappedIter:
            if AsyncUtil.is_async_code:
                ...
                if not inspect.isgeneratorfunction(f):
                    raise TypeError(
                        "cannot decorate non-generator function with " "NonConcurrentMethodChecked.non_concurrent_iter"
                    )

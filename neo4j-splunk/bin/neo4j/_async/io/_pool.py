...
class AsyncIOPool(abc.ABC):
    ...
    def _acquire_new_later(self, address, auth, deadline):
        async def connection_creator():
            ...
            try:
                try:
                    connection = await self.opener(address, auth or self.pool_config.auth, deadline)
                ...
                if auth:
                    ...
                    try:
                        ...
                        log.debug("[#%04X]  _: <POOL> no re-auth support", connection.local_port)
                        await connection.close()
                        raise
                connection.pool = self
                connection.in_use = True
        ...
        with self.lock:
            ...
            pool_size = len(connections) + self.connections_reservations[address]
    ...
    async def _re_auth_connection(self, connection, auth, force):
        ...
        try:
            updated = connection.re_auth(new_auth, new_auth_manager, force=force)
            log.debug(
                "[#%04X]  _: <POOL> checked re_auth auth=%s updated=%s " "force=%s",
                connection.local_port,
                log_auth,
                updated,
                force,
    ...
    async def _acquire(self, address, auth, deadline, liveness_check_timeout):
        ...
        async def health_check(connection_, deadline_):
            if connection_.closed() or connection_.defunct() or connection_.stale():
                return False
            if liveness_check_timeout is not None:
                if connection_.is_idle_for(liveness_check_timeout):
                    with connection_deadline(connection_, deadline_):
                        try:
                            log.debug("[#%04X]  _: <POOL> liveness check", connection_.local_port)
        ...
        while True:
            ...
            connection = await self._acquire_from_pool_checked(address, health_check, deadline)
            if connection:
                log.debug(
                    "[#%04X]  _: <POOL> picked existing connection %s", connection.local_port, connection.connection_id
                )
                try:
                    await self._re_auth_connection(connection, auth, force_auth)
                except ConfigurationError:
                    if auth:
                    ...
                    log.debug(
                        "[#%04X]  _: <POOL> backwards compatible " "auth token refresh: purge connection",
                        connection.local_port,
                    )
                    await connection.close()
                    await self.release(connection)
                    continue
                log.debug("[#%04X]  _: <POOL> handing out existing connection", connection.local_port)
                return connection
            ...
            with self.lock:
                ...
                if timeout == 0 or not await self.cond.wait(timeout):  # deadline expired
    ...
    @abc.abstractmethod
    async def acquire(self, access_mode, timeout, database, bookmarks, auth: AcquireAuth, liveness_check_timeout):
    ...
    async def release(self, *connections):
        ...
        cancelled = None
        for connection in connections:
            if not (connection.defunct() or connection.closed() or connection.is_reset):
                if cancelled is not None:
                    log.debug(
                        "[#%04X]  _: <POOL> kill unclean connection %s", connection.local_port, connection.connection_id
                    )
                    connection.kill()
                    continue
                try:
                    ...
                    log.debug(
                        "[#%04X]  _: <POOL> failed to reset connection " "on release: %r", connection.local_port, exc
                    )
                except asyncio.CancelledError as exc:
                    log.debug(
                        "[#%04X]  _: <POOL> cancelled reset connection " "on release: %r", connection.local_port, exc
                    )
                    cancelled = exc
                    connection.kill()
        with self.lock:
            for connection in connections:
                connection.in_use = False
                log.debug("[#%04X]  _: <POOL> released %s", connection.local_port, connection.connection_id)
            self.cond.notify_all()
        if cancelled is not None:
            raise cancelled
    ...
    async def deactivate(self, address):
        ...
        with self.lock:
            try:
                ...
                return
            closable_connections = [conn for conn in connections if not conn.in_use]
    ...
    async def on_neo4j_error(self, error, connection):
        ...
        if error._unauthenticates_all_connections():
            address = connection.unresolved_address
            log.debug("[#0000]  _: <POOL> mark all connections to %r as " "unauthenticated", address)
            with self.lock:
        ...
        if error._has_security_code():
            handled = await AsyncUtil.callback(
                connection.auth_manager.handle_security_exception, connection.auth, error
            )
            if handled:
...
class AsyncBoltPool(AsyncIOPool):
    ...
    def open(cls, address, *, pool_config, workspace_config):
        ...
        async def opener(addr, auth_manager, deadline):
            return await AsyncBolt.open(
                addr, auth_manager=auth_manager, deadline=deadline, routing_context=None, pool_config=pool_config
            )
    ...
    async def acquire(self, access_mode, timeout, database, bookmarks, auth: AcquireAuth, liveness_check_timeout):
        ...
        log.debug("[#0000]  _: <POOL> acquire direct connection, " "access_mode=%r, database=%r", access_mode, database)
        deadline = Deadline.from_timeout_or_deadline(timeout)
        return await self._acquire(self.address, auth, deadline, liveness_check_timeout)
...
class AsyncNeo4jPool(AsyncIOPool):
    ...
    @classmethod
    def open(cls, *addresses, pool_config, workspace_config, routing_context=None):
    ...
    async def get_or_create_routing_table(self, database):
        async with self.refresh_lock:
            if database not in self.routing_tables:
                self.routing_tables[database] = RoutingTable(database=database, routers=[self.address])
            return self.routing_tables[database]

    async def fetch_routing_info(self, address, database, imp_user, bookmarks, auth, acquisition_timeout):
        ...
        log.debug("[#0000]  _: <POOL> _acquire router connection, " "database=%r, address=%r", database, address)
        if auth:
            auth = copy(auth)
    ...
    async def fetch_routing_table(self, *, address, acquisition_timeout, database, imp_user, bookmarks, auth):
        ...
        try:
            new_routing_info = await self.fetch_routing_info(
                address, database, imp_user, bookmarks, auth, acquisition_timeout
            )
        ...
        if not new_routing_info:
            log.debug("[#0000]  _: <POOL> failed to fetch routing info " "from %r", address)
            return None
        else:
        ...
        # No routers
        if num_routers == 0:
            log.debug("[#0000]  _: <POOL> no routing servers returned from " "server %s", address)
            return None

        # No readers
        if num_readers == 0:
            log.debug("[#0000]  _: <POOL> no read servers returned from " "server %s", address)
            return None
    ...
    async def _update_routing_table_from(
        self, *routers, database, imp_user, bookmarks, auth, acquisition_timeout, database_callback
    ):
        ...
        if routers:
            log.debug(
                "[#0000]  _: <POOL> attempting to update routing " "table from {}".format(", ".join(map(repr, routers)))
            )
        for router in routers:
            async for address in AsyncNetworkUtil.resolve_address(router, resolver=self.pool_config.resolver):
                ...
                if new_routing_table is not None:
                    ...
                    old_routing_table = await self.get_or_create_routing_table(new_database)
                    old_routing_table.update(new_routing_table)
                    log.debug(
                        "[#0000]  _: <POOL> update routing table from " "address=%r (%r)",
                        address,
    ...
    async def update_routing_table(
        self, *, database, imp_user, bookmarks, auth=None, acquisition_timeout=None, database_callback=None
    ):
        ...
        async with self.refresh_lock:
            ...
            prefer_initial_routing_address = self.routing_tables[database].initialized_without_writers
    ...
    async def ensure_routing_table_is_fresh(
        self, *, access_mode, database, imp_user, bookmarks, auth=None, acquisition_timeout=None, database_callback=None
    ):
    ...
    async def acquire(
        self, access_mode, timeout, database, bookmarks, auth: t.Optional[AcquireAuth], liveness_check_timeout
    ):
        ...
        log.debug(
            "[#0000]  _: <POOL> acquire routing connection, " "access_mode=%r, database=%r", access_mode, database
        )
        await self.ensure_routing_table_is_fresh(
        ...
        while True:
            try:
                ...
                # connections.
                address = await self._select_address(access_mode=access_mode, database=database)
            ...
            try:
                log.debug("[#0000]  _: <POOL> acquire address, database=%r " "address=%r", database, address)
                deadline = Deadline.from_timeout_or_deadline(timeout)
    ...
    async def on_write_failure(self, address, database):
        ...
        log.debug("[#0000]  _: <POOL> removing writer %r for database %r", address, database)
        async with self.refresh_lock:

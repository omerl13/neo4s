    ...
    async def _acquire(self, address, auth, deadline, liveness_check_timeout):
        ...
        while True:
            ...
            with self.lock:
                ...
                if timeout == 0 or not await self.cond.wait(  # deadline expired
                    timeout
                ):
    ...
    async def on_neo4j_error(self, error, connection):
        ...
        if error._requires_new_credentials():
            await AsyncUtil.callback(
                connection.auth_manager.on_auth_expired, connection.auth
            )
        if isinstance(error, TokenExpired) and not isinstance(
            self.pool_config.auth, (AsyncStaticAuthManager, StaticAuthManager)
        ):

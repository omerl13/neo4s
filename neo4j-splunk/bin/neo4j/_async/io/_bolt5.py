...
class AsyncBolt5x0(AsyncBolt):
    ...
    def _on_server_state_change(self, old_state, new_state):
        log.debug("[#%04X]  _: <CONNECTION> server state: %s > %s", self.local_port, old_state.name, new_state.name)
    ...
    def _on_client_state_change(self, old_state, new_state):
        log.debug("[#%04X]  _: <CONNECTION> client state: %s > %s", self.local_port, old_state.name, new_state.name)
    ...
    async def hello(self, dehydration_hooks=None, hydration_hooks=None):
        if self.notifications_min_severity is not None or self.notifications_disabled_categories is not None:
        ...
        def on_success(metadata):
            ...
            if "connection.recv_timeout_seconds" in self.configuration_hints:
                recv_timeout = self.configuration_hints["connection.recv_timeout_seconds"]
        ...
        self._append(
            b"\x01",
            (headers,),
            response=InitResponse(self, "hello", hydration_hooks, on_success=on_success),
    ...
    def telemetry(self, api: TelemetryAPI, dehydration_hooks=None, hydration_hooks=None, **handlers) -> None:
        # TELEMETRY not support by this protocol version, so we ignore it.
        pass

    async def route(self, database=None, imp_user=None, bookmarks=None, dehydration_hooks=None, hydration_hooks=None):
        ...
        log.debug("[#%04X]  C: ROUTE %r %r %r", self.local_port, routing_context, bookmarks, db_context)
        metadata = {}
        ...
        self._append(
            ...
            response=Response(self, "route", hydration_hooks, on_success=metadata.update),
    ...
    def run(
        ...
        **handlers,
        ...
        if notifications_min_severity is not None or notifications_disabled_categories is not None:
            self.assert_notification_filtering_support()
        if not parameters:
            parameters = {}
        extra = {}
        ...
        if db:
            extra["db"] = db
        if self._client_state_manager.state != self.bolt_states.TX_READY_OR_TX_STREAMING:
        ...
        self._append(
            b"\x10", fields, Response(self, "run", hydration_hooks, **handlers), dehydration_hooks=dehydration_hooks
        )
    ...
    def pull(self, n=-1, qid=-1, dehydration_hooks=None, hydration_hooks=None, **handlers):
        extra = {"n": n}
        if qid != -1:
            extra["qid"] = qid
        log.debug("[#%04X]  C: PULL %r", self.local_port, extra)
        self._append(
            b"\x3F", (extra,), Response(self, "pull", hydration_hooks, **handlers), dehydration_hooks=dehydration_hooks
        )
    ...
    def begin(
        ...
        **handlers,
        ...
        self._append(
            b"\x11", (extra,), Response(self, "begin", hydration_hooks, **handlers), dehydration_hooks=dehydration_hooks
        )
    ...
    def rollback(self, dehydration_hooks=None, hydration_hooks=None, **handlers):
        ...
        self._append(
            b"\x13", (), Response(self, "rollback", hydration_hooks, **handlers), dehydration_hooks=dehydration_hooks
        )
    ...
    async def reset(self, dehydration_hooks=None, hydration_hooks=None):
        ...
        self._append(b"\x0F", response=response, dehydration_hooks=dehydration_hooks)
        await self.send_all()
        await self.fetch_all()
    ...
    async def _process_message(self, tag, fields):
        ...
        if summary_signature == b"\x70":
            ...
            try:
                ...
                if self.pool:
                    await self.pool.on_write_failure(address=self.unresolved_address, database=self.last_database)
                raise
            ...
            raise BoltProtocolError(
                "Unexpected response message with signature %02X" % ord(summary_signature), self.unresolved_address
            )
...
class AsyncBolt5x1(AsyncBolt5x0):
    ...
    def logoff(self, dehydration_hooks=None, hydration_hooks=None):
        ...
        self._append(
            b"\x6B", response=LogonResponse(self, "logoff", hydration_hooks), dehydration_hooks=dehydration_hooks
        )
...
class AsyncBolt5x2(AsyncBolt5x1):
    ...
    def get_base_headers(self):
        ...
        if self.notifications_disabled_categories is not None:
            headers["notifications_disabled_categories"] = self.notifications_disabled_categories
        return headers
    ...
    def run(
        ...
        **handlers,
        ...
        if notifications_disabled_categories is not None:
            extra["notifications_disabled_categories"] = notifications_disabled_categories
        if bookmarks:
            try:
                extra["bookmarks"] = list(bookmarks)
            except TypeError:
...
class AsyncBolt5x4(AsyncBolt5x3):
    ...
    def telemetry(self, api: TelemetryAPI, dehydration_hooks=None, hydration_hooks=None, **handlers) -> None:
        if self.telemetry_disabled or not self.configuration_hints.get("telemetry.enabled", False):
            return
        api_raw = int(api)
        log.debug("[#%04X]  C: TELEMETRY %i  # (%r)", self.local_port, api_raw, api)
        self._append(
            b"\x54",
            (api_raw,),

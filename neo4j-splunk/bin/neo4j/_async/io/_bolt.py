...
class AsyncBolt:
    ...
    def __init__(
        ...
        *,
        ...
        self.server_info = ServerInfo(
            ResolvedAddress(sock.getpeername(), host_name=unresolved_address.host), self.PROTOCOL_VERSION
        )
        ...
        self.patch = {}
        self.outbox = AsyncOutbox(self.socket, on_error=self._set_defunct_write, packer_cls=self.PACKER_CLS)
        self.inbox = AsyncInbox(self.socket, on_error=self._set_defunct_read, unpacker_cls=self.UNPACKER_CLS)
    ...
    # [bolt-version-bump] search tag when changing bolt version support
    @classmethod
    async def open(cls, address, *, auth_manager=None, deadline=None, routing_context=None, pool_config=None):
        ...
        try:
            ...
            log.debug("[#%04X]  C: <KILL> open auth manager failed: %r", s.getsockname()[1], e)
            s.kill()
            raise
        except Exception as e:
            log.debug("[#%04X]  C: <CLOSE> open auth manager failed: %r", s.getsockname()[1], e)
            await s.close()
            raise
    ...
    def re_auth(
        ...
        if not force and new_auth_dict == self.auth_dict:
            ...
            self.auth = auth
            return False
        self.logoff(dehydration_hooks=dehydration_hooks, hydration_hooks=hydration_hooks)
    ...
    async def fetch_message(self):
        ...
        tag, fields = await self.inbox.pop(hydration_hooks=self.responses[0].hydration_hooks)
    ...
    def stale(self):
        return self._stale or (0 <= self._max_connection_lifetime <= monotonic() - self._creation_timestamp)
    ...
    async def close(self):
        ...
        if not self._defunct:
            ...
            try:
                ...
                log.debug("[#%04X]  _: <CONNECTION> ignoring failed close %r", self.local_port, exc)
    ...
    def kill(self):
        ...
        try:
            ...
            log.debug("[#%04X]  _: <CONNECTION> ignoring failed kill %r", self.local_port, exc)
        finally:
            self._closed = True

...
class ServerStateManager(ServerStateManagerBase):
    ...
    def transition(self, message, metadata):
        ...
        self.state = self._STATE_TRANSITIONS.get(self.state, {}).get(message, self.state)
...
class AsyncBolt3(AsyncBolt):
    ...
    async def route(self, database=None, imp_user=None, bookmarks=None, dehydration_hooks=None, hydration_hooks=None):
        if database is not None:
            raise ConfigurationError(
                ...
                "Server Agent {!r}".format(self.PROTOCOL_VERSION, database, self.server_info.agent)
            )
        ...
        self.pull(dehydration_hooks=None, hydration_hooks=None, on_success=metadata.update, on_records=records.extend)
        await self.send_all()
        await self.fetch_all()
    ...
    def run(
        ...
        **handlers,
        ...
        if db is not None:
            raise ConfigurationError(
                "Database name parameter for selecting database is not "
                "supported in Bolt Protocol {!r}. Database name {!r}.".format(self.PROTOCOL_VERSION, db)
            )
        if imp_user is not None:
            raise ConfigurationError(
                "Impersonation is not supported in Bolt Protocol {!r}. "
    ...
    def discard(self, n=-1, qid=-1, dehydration_hooks=None, hydration_hooks=None, **handlers):
        ...
        self._append(
            b"\x2F", (), Response(self, "discard", hydration_hooks, **handlers), dehydration_hooks=dehydration_hooks
        )
    ...
    def pull(self, n=-1, qid=-1, dehydration_hooks=None, hydration_hooks=None, **handlers):
        ...
        self._append(
            b"\x3F", (), Response(self, "pull", hydration_hooks, **handlers), dehydration_hooks=dehydration_hooks
        )
    ...
    async def _process_message(self, tag, fields):
        ...
        if summary_signature == b"\x70":
            ...
            raise BoltProtocolError(
                "Unexpected response message with signature %02X" % summary_signature, address=self.unresolved_address
            )

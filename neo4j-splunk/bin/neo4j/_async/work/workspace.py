...
class AsyncWorkspace(AsyncNonConcurrentMethodChecker):
    ...
    async def _get_bookmarks(
        ...
        self._last_from_bookmark_manager = tuple(
            {*await AsyncUtil.callback(self._bookmark_manager.get_bookmarks), *self._initial_bookmarks}
        )
    ...
    async def _update_bookmarks(self, new_bookmarks):
        ...
        await AsyncUtil.callback(self._bookmark_manager.update_bookmarks, previous_bookmarks, new_bookmarks)
    ...
    async def _connect(self, access_mode, auth=None, **acquire_kwargs):
        ...
        if not self._cached_database:
            if self._config.database is not None or not isinstance(self._pool, AsyncNeo4jPool):

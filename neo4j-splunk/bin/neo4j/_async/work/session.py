...
class AsyncSession(AsyncWorkspace):
    ...
    async def _connect(self, access_mode, **acquire_kwargs):
        ...
        try:
            await super()._connect(access_mode, auth=self._config.auth, **acquire_kwargs)
    ...
    def _handle_cancellation(self, message="General"):
        ...
        if connection:
            log.debug("[#%04X]  _: <SESSION> %s cancellation clean-up", connection.local_port, message)
    ...
    async def run(
        ...
        **kwargs: t.Any,
        ...
        self._auto_result = AsyncResult(cx, self._config.fetch_size, self._result_closed, self._result_error)
    ...
    async def _open_transaction(
        self,
        *,
        tx_cls: t.Callable[..., t.Union[AsyncTransaction, AsyncManagedTransaction]],
        access_mode,
        api: t.Optional[TelemetryAPI],
        metadata=None,
        timeout=None,
    ...
    async def begin_transaction(
        self, metadata: t.Optional[t.Dict[str, t.Any]] = None, timeout: t.Optional[float] = None
    ) -> AsyncTransaction:
        ...
        if self._transaction:
            raise TransactionError(self._transaction, "Explicit transaction already open")
    ...
    async def _run_transaction(
        ...
        access_mode: str,
        api: TelemetryAPI,
        transaction_function: t.Callable[te.Concatenate[AsyncManagedTransaction, _P], t.Awaitable[_R]],
        args: _P.args,
        kwargs: _P.kwargs,
    ) -> _R:
        self._check_state()
        ...
        while True:
            ...
            log.warning(
                "Transaction failed and will be retried in {}s ({})" "".format(delay, "; ".join(errors[-1].args))
            )
            try:
    ...
    async def execute_read(
        ...
        **kwargs: _P.kwargs,
        ...
        .. versionadded:: 5.0
        """
        return await self._run_transaction(READ_ACCESS, TelemetryAPI.TX_FUNC, transaction_function, args, kwargs)
    ...
    async def execute_write(
        ...
        **kwargs: _P.kwargs,
        ...
        .. versionadded:: 5.0
        """
        return await self._run_transaction(WRITE_ACCESS, TelemetryAPI.TX_FUNC, transaction_function, args, kwargs)

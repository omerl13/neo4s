...
class AsyncStaticAuthManager(AsyncAuthManager):
    ...
    async def handle_security_exception(self, auth: _TAuth, error: Neo4jError) -> bool:
        return False
...
class AsyncNeo4jAuthTokenManager(AsyncAuthManager):
    ...
    def __init__(self, provider: t.Callable[[], t.Awaitable[ExpiringAuth]], handled_codes: t.FrozenSet[str]) -> None:
    ...
    async def _refresh_auth(self):
        ...
        if self._current_auth is None:
            raise TypeError(
                "Auth provider function passed to expiration_based " "AuthManager returned None, expected ExpiringAuth"
            )
    ...
    async def get_auth(self) -> _TAuth:
        async with self._lock:
            ...
            if auth is None or expiring_auth_has_expired(auth):
                log.debug("[     ]  _: <AUTH MANAGER> refreshing (%s)", "init" if auth is None else "time out")
    ...
    async def handle_security_exception(self, auth: _TAuth, error: Neo4jError) -> bool:
        ...
        async with self._lock:
            ...
            if cur_auth is not None and cur_auth.auth == auth:
                log.debug("[     ]  _: <AUTH MANAGER> refreshing (error %s)", error.code)
                await self._refresh_auth()
            return True
...
class AsyncAuthManagers:
    ...
    @staticmethod
    def basic(provider: t.Callable[[], t.Awaitable[_TAuth]]) -> AsyncAuthManager:
    ...
    @staticmethod
    def bearer(provider: t.Callable[[], t.Awaitable[ExpiringAuth]]) -> AsyncAuthManager:
